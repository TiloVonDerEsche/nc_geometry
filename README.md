...
6. **Start the `generate_track_list.exe`** in a terminal.
   Afterwards, `track_list.csv` should appear at the path, specified through the `track_list_csv` var.

7. Make sure `3d_track_visualizer.exe` is in the **same directory** as `generate_track_list.exe`.  
   Both should have the `data` folder, with the CSVs and the `.mpf` file in their directory.

8. Start `3d_track_visualizer.exe` — this should open a window with a 3D visualization of the tracks.  
