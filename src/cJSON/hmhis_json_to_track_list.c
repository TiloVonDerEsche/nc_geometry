...
int main() {
    ...
    char* hmhis_path = "./data/hmhis.json";
    printf("Opening %s in binary read mode...\n",hmhis_path);
    FILE *f = fopen(hmhis_path, "rb");
    if (!f) {
         printf("Failed to open %s in binary read mode!\n",hmhis_path);
        return 1;    
    } 
    fseek(f, 0, SEEK_END);
    long len = ftell(f);
    ...
    char* track_list_path = "./data/track_list.csv";
    printf("Opening %s in write mode...\n",track_list_path);
    FILE *csv = fopen(track_list_path, "w");
